...
void IQSwipeTask(CGPoint pstart , CGPoint pEnd) {
    IQUITestOperationEvent *op = [IQUITestOperationEvent new];
...
@interface UIScrollView (IQRunTimeHook)
@property(nonatomic,assign) CGPoint pstart;
...
- (void)IQ_setDelegate:(id<UIScrollViewDelegate>)delegate {
    ...
    Method currentMethod = class_getInstanceMethod([self class], @selector(IQ_scrollViewWillBeginDragging:));
    IMP currentImp = method_getImplementation(currentMethod);

    class_addMethod([delegate class], @selector(IQ_scrollViewWillBeginDragging:), currentImp, method_getTypeEncoding(currentMethod));

    BOOL didAddMethod = class_addMethod([delegate class], @selector(scrollViewWillBeginDragging:), currentImp, method_getTypeEncoding(currentMethod));

    if (didAddMethod) {
        class_replaceMethod([delegate class], @selector(IQ_scrollViewWillBeginDragging:), originImp, method_getTypeEncoding(originMethod));
    } else {
    ...
    Method originMethod1 = class_getInstanceMethod([delegate class], @selector(scrollViewDidEndDecelerating:));
    IMP originImp1 = method_getImplementation(originMethod1);
    
    Method currentMethod1 = class_getInstanceMethod([self class], @selector(IQ_scrollViewDidEndDecelerating:));
    IMP currentImp1 = method_getImplementation(currentMethod1);
    
    class_addMethod([delegate class], @selector(IQ_scrollViewDidEndDecelerating:), currentImp1, method_getTypeEncoding(currentMethod1));
    
    BOOL didAddMethod1 = class_addMethod([delegate class], @selector(scrollViewDidEndDecelerating:), currentImp1, method_getTypeEncoding(currentMethod1));
    
    if (didAddMethod1) {
        class_replaceMethod([delegate class], @selector(IQ_scrollViewDidEndDecelerating:), originImp1, method_getTypeEncoding(originMethod1));
    } else {
...
- (void)IQ_scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    [self IQ_scrollViewWillBeginDragging:scrollView];
//    self.pstart = [scrollView contentOffset];
    objc_setAssociatedObject(self, @selector(IQ_scrollViewWillBeginDragging:), @(scrollView.contentOffset), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
